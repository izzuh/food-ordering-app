...
export async function listRestaurantOrders(ownerId: string) {
  const result = await db.query(
    `SELECT o.id, o.restaurant_id, o.status, o.payment_status,
            o.subtotal_minor, o.delivery_fee_minor, o.total_minor, o.currency,
            o.delivery_address_snapshot, o.created_at
...
export async function getRestaurantOrder(ownerId: string, orderId: string) {
  const order = await db.query(
    `SELECT o.id, o.restaurant_id, o.status, o.payment_status,
            ...
            o.delivery_address_snapshot, o.created_at
     FROM orders o JOIN restaurants r ON r.id = o.restaurant_id
     WHERE o.id = $1 AND r.owner_id = $2 LIMIT 1`,
    [orderId, ownerId],
  );
...
export async function updateRestaurantOrderStatus(ownerId: string, orderId: string, status: string) {
  const result = await db.query(
    `UPDATE orders o SET status = $3, updated_at = NOW()
     FROM restaurants r
     WHERE o.id = $1 AND o.restaurant_id = r.id AND r.owner_id = $2
     RETURNING o.id, o.status`,
    [orderId, ownerId, status],
  );
