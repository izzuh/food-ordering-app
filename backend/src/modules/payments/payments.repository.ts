...
export async function createPayment(input: {
  ...
  const result = await db.query(
    `INSERT INTO payments (order_id, user_id, provider, reference, amount_minor, currency, status)
     VALUES ($1, $2, 'paystack', $3, $4, 'NGN', 'pending')
...
export async function findPaymentByReference(reference: string) {
  const result = await db.query(
    `SELECT id, order_id, user_id, provider, reference, amount_minor, currency, status
...
export async function markPaymentPaid(paymentId: string, orderId: string) {
  ...
  try {
    ...
    await client.query(`UPDATE payments SET status = 'paid', paid_at = NOW(), updated_at = NOW() WHERE id = $1`, [paymentId]);
    await client.query(`UPDATE orders SET payment_status = 'paid', status = 'confirmed', updated_at = NOW() WHERE id = $1 AND payment_status <> 'paid'`, [orderId]);
...
export async function markPaymentFailed(paymentId: string) {
  await db.query(`UPDATE payments SET status = 'failed', updated_at = NOW() WHERE id = $1 AND status <> 'paid'`, [paymentId]);
}
