...
export default function LoginScreen({ onAuthenticated }: { onAuthenticated: (token: string) => void }) {
  ...
  async function submit() {
    ...
    try { const result = await login({ email: email.trim(), password }); onAuthenticated(result.tokens.accessToken); }
    catch { setError('Unable to sign in. Check your email and password.'); }
    finally { setLoading(false); }
  }
  return <View style={styles.container}>
    <Text style={styles.title}>Welcome back</Text><Text style={styles.subtitle}>Sign in to order your favourite food.</Text>
